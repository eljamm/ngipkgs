...
### CI/CD
- Update packages with flake update
- Update changelog
...
- Improve output of packaging report script
- Don't link to demo
- Remove redunadant information

### Features
- Count maintained derivations in Nixpkgs
- Enforce subgrant structure in metadata
...
- Update usage instructions by demo type
...
### Project
- Remove upstreamed tests depscan.nix, blint.nix

### Projects
- Refactor config & test; add verbosity
- Mark demo test as broken
- Fix docs link
- Fix docs links
- Remove docs link
...
- Add ubuntu 25.04 to test matrix
...
* overview: add Nix installation instructions for Linux distros

Closes: #942

* overview: add service demo instructions for older Nix versions

Closes: #943

* overview: add service demo instructions for adding binary substituters
...
### Overview
- Reword motivation and repo overview in README (#777)
- Fix wording (#636)
- Remove packages
- Remove `deploy` directory

Since it's outdated and will be deprecated by the overview, which will
detail how to install and deploy NGIpkgs projects.
- Move to `overview`
...
- Fix derivation expected for test

If a test returns a derivation (like Forgejo), use that directly instead
of trying to call `nixosTest` on it.
...
### Overview
- Move projects to projects-old; add compatability mapping

We will incrementally migrate the old projects into the new type-checked
format until the directory is empty. Until then, a compatibility mapping
was added to convert new projects to the old format in order to preserve
existing functionality with other parts (like the overview).
- Show program modules in the overview (#560)
- Stable project urls (#570)
- Map metadata in projects (#580)

which will be used in the overview.
